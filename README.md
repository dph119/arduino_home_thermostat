...
## Introduction
    ...
    to create this Arduino setup where I can keep track of the temperature in the room
    and simply adjust the thermostat accordingly.
...
## Materials
    1. Two (2) Arduino Uno
        1. One of them could probably be smaller.
...
## Organization
    This whole thing is broken up into two parts: the part controlling the thermostat, and the part
...
### Thermostat/Main Server (thermostat_arduino.ino and thermostat_esp8266.ino)
    ...
    The thermostat is an old one of the [Honeywell T87F](https://customer.honeywell.com/resources/techlit/TechLitDocuments/60-0000s/60-2222.pdf) variety. The heat is turned on when the
    circuit is "completed" by a mercury switch.
    
    The idea was to have the wheel driven by the stepper motor shoved up against the thermostat.
    I considered tapping right into the wires used by the thermostat to turn on the heat,
    but figured this approach would leave the door open to being able to program
    the thermostat. 
    ...
    the other guy for a temperature, and keep some history and an average temperature.
    ...
    motor set the thermostat to 80 degrees fahrenheit. Yeah I could've setup up a DNS, but I didn't really care.
...
### Temperature Client (temperature_client_arduino.ino and temperature_client_esp8266.ino)
    ...
    The ESP8266 is set to AP_STA mode. This was originally so that I could directly
    connect to this guy and view the temperature myself, but it's practically deprecated
    now that the Thermostat Arduino can serve up a summary of everything, including the temperature
    reported here.
...
## Gotchas
    Here's a list of stupid things I wasted time on.

### Programming the ESP8266
    1. For the particular kind I was using, you need to connect the GP100 pin to GND to specifically tell it you want to program it.
    ...
    when the chip gets super hot and possibly starts to reboot itself repeatedly.
    
    5. The TX pin is connected to the TX pin of the Arduino (and RX to RX) for programming. But when you're using it
...
### Communicating with the ESP8266
    1. When the Arduino and ESP8266 send messages to each other over the Serial connection, they pretty much
    exclusively use the "readStringUntil()" function. The delimiter I used is is '\n'. I made the unfortunate mistake of
    assuming that Serial.println() terminates their strings with ONLY a NEWLINE charater ('\n'), when in fact
    it also includes the carriage return '\r'. So there were issues where my commands weren't being "recognized"
    because they contained an invisible carriage return. 
...
### Driving the Stepper Motor
    ...
    the outer part of the USB connected touch the connector on the Arduino, the motor suddenly worked, pointing
    to a grounding issue. The motor produced the expected torque (and was generally snappier) when I added a jumper
    between the ground pins.
...
### BMP180 and Enclosed Spaces
    ...
    lives in together. I quickly started to notice that the reported temperature was 5-10 degrees higher
    than what it really was in the room. It quickly become obvious that the heat generated by the ESP8266 in
    particular was getting in the way. I eventually had to make another opening for the BMP180 to poke out.
            
## Misc. Thermostat Notes
    My thermostat only triggers the heat. There is no cooling system. In general, it's going to take
    more time for a room to cool off than it does to heat up. So, when we make adjustments to the thermostat,
    we need to account for that. When we turn up the heat, we should be ready to turn down the thermostat
    more quickly.
