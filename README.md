...
9. One (1) LCD and Keypad Shield
    1. [OSEPP 16x2 LCD and Keypad shield](https://www.osepp.com/electronic-modules/shields/45-16-2-lcd-display-keypad-shield)
...
Later on I decided to add an LCD and keypad to this setup. The shield I used has a
slightly tweaked version of the common LCD library provided for the Arduino (grabbed from
the manufacture's website). Anyway, the sample code was tweaked support searching for button
presses but timing out, as well as some minor "debouncing" code. The display has multiple
menus, the choice of which is controlled via a state machine. From these menus
the thermostat temperature or desired temperature can be modified, and the new proposed value
is stored temporarily until that menu is exited. The big motivation for waiting until
the menu is exited is to avoid having the stepper motor adjust the thermostat by 1 degree
as you press the buttons to set the new temperature you want.

If you modify any of the variables via the URL commands, the display will pick it up accordingly.
The code was restructured slightly to have the display-related code in its own file, but the common
temperature variables in a "common" file. Basically the main Arduino board code and the display
code need to play nice with each other, since they can both modify the variables and
make calls to adjust the thermostat itself.        
...
### Pin Connections for the OSEPP 16x2 LCD/Keypad
The LCD/keypad shield has some room for soldering on pin headers so you can use the remaining unused pins.
If you look at the physical shield, there is one set of pins whose location is right along the
Arduino Uno's digital pins 0-7. It turns out that the provided pins don't actually map to that,
and I didn't see this mentioned anywhere clearly. I ended up having to test the connectivity of the
header pins to the soldered points underneath the shield. It turns out pins 0-3 are connected
to the original digital pins 0-3, but the remaining pins of the header (what I thought were 4-6)
are actually connected to digital pins 11-13. Point being, don't make assumptions about the pin layout
on shields.

### Analog Inputs can be Used as Digital Outputs
When I added the LCD/Keypad, I was running out of output pins. I didn't think the pins labeled "Analog In"
could be used for anything else, but it turns out that you can treat them as additional digital output
pins. The actual pin numbers just continue from the last "real" digital pins. For example, A0 is mapped
to digital pin 14, A1 is mapped to digital pin 15, etc. It didn't matter in my case, but these pins don't
support PWM though. You could emulate it if you wanted to.    
